...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'      # 정보 출력 정도
os.environ["CUDA_DEVICE_ORDER"]="PCI_BUS_ID"    # ???
os.environ["CUDA_VISIBLE_DEVICES"]= "0"     # 사용할 gpu 할당
...
# tfis = tfgan.eval.inception_score()
...
##############
####################################################################
    n_cpus = multiprocessing.cpu_count()    # cpu 갯수 확인
    print(n_cpus)
    pool = multiprocessing.Pool(processes=n_cpus-3) if args.use_pool else None
    ...
    # distribute.MirroredStrategy 분산 처리
    ...
    for k in range(args.n_policies):
        policy_train_ = get_policy(args, op_names=augmentation_search.op_names, 
                                   ops_mid_magnitude=ops_mid_magnitude, 
    ...
    def train_policy_stage1(stage, images_val_, labels_val_, images_batch, labels_batch):
        ...
        if isinstance(images_batch[0], list):
            images_aug_last, labels_aug_last = augmentation_search(
                                                                    repeat(sum(images_batch,[]), len(ops_dense), axis=0), 
                                                                    repeat(np.concatenate(labels_batch), len(ops_dense), axis=0),
                                                                    np.tile(ops_dense, [search_bs * EXP, 1]), 
                                                                    np.tile(mags_dense, [search_bs * EXP, 1]).astype(np.float32)/float(args.l_mags-1),
